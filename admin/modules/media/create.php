...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!validate_csrf_token($csrf_token)) {
    ...
    if (empty($errors)) {
      try {
        $stmt = $pdo->prepare("INSERT INTO gallery (title, image, category, publish_date, display_section, status, uploaded_by, created_at) VALUES (?, ?, ?, ?, 'media_coverage', ?, ?, NOW())");
        $stmt->execute([$title, $image, $category, $publish_date, $status, $_SESSION['admin_id']]);
        ...
        <form method="POST" action="" enctype="multipart/form-data">
          ...
          <div class="mb-3">
            ...
            <input type="date" class="form-control" id="publish_date" name="publish_date"
              value="<?php echo escape($_POST['publish_date'] ?? ''); ?>" required>
          </div>
